...
- `claude/` - Claude Code global config mirroring `~/.claude/`: `CLAUDE.md`, `RTK.md`, `settings.json`, `hooks/`, plus the bundled `repo-docs`, `dev-core`, `concise-output`, `rules-index`, `orchestrate`, and `better-design` plugins. `claude/install.sh` installs just this adapter; `./install.sh` covers both.
