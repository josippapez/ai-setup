...
- `claude/` - Claude Code global config mirroring `~/.claude/`: `CLAUDE.md`, `RTK.md`, `settings.json`, `hooks/`, plus the bundled `repo-docs`, `dev-core`, `concise-output`, `rules-index`, `orchestrate`, `better-design`, and `fundamentals` plugins. `claude/install.sh` installs just this adapter; `./install.sh` covers both.
...
The free-tier OpenCode agents (`free-tier-*`, `repo-scout`, `docs-maintainer`, `md-worker-free`) pin an OpenCode Zen free model. That list rotates and models get retired, so when one stops resolving, pick a replacement from the current free list (`curl -s https://opencode.ai/zen/v1/models`, `/models` in the TUI, or the pricing table at <https://opencode.ai/docs/zen/>) and update the `model:` frontmatter in those files. Free Zen models allow the provider to use your prompts, so keep confidential repos off them.
...
- The `PostToolUse` (`Edit`/`Write`/`Bash`) hook runs `format-lint-edited-files.mjs`: Prettier (`--write`) on supported files (`.ts`, `.tsx`, `.js`, `.jsx`, `.json`, `.md`, `.css`, `.scss`, `.html`, `.yml`, `.yaml`) and, because `settings.json` sets `HOOK_RUN_ESLINT=1`, ESLint (`--fix --max-warnings=0`) on JS/TS files. Both run from each project's local `node_modules/.bin`; the hook is best-effort and silently no-ops when the tool isn't installed for that project. It matches `Bash` as well as the edit tools because most file writing goes through the shell (`sed -i`, `tee`, `cat > f <<EOF`), which carries no `file_path`; those three shapes are parsed out of the command, and a path the command named but did not create is ignored. Tests: `node --test claude/hooks/scripts/*.test.mjs`.
