...
- `claude/` - Claude Code global config mirroring `~/.claude/`: `CLAUDE.md`, `RTK.md`, `settings.json`, `hooks/`, plus the bundled `interactive-mcp` and `markdown-orchestration` plugins. Run `claude/install.sh` to install it globally.
...
`claude/` mirrors `~/.claude/` and includes a local Claude marketplace (repo-root `.claude-plugin/marketplace.json`) with `interactive-mcp` and `markdown-orchestration`; orchestration auto-loads only its compact dispatcher, which explicitly reads bundled routing/store/phase/platform references and templates on demand by absolute skill-root path.
