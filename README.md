...
- `claude/` - Claude Code global config mirroring `~/.claude/`: `CLAUDE.md`, `RTK.md`, `settings.json`, `hooks/`, plus the bundled `dev-core`, `concise-output`, and `markdown-orchestration` plugins. `claude/install.sh` installs just this adapter; `./install.sh` covers both.
