...
- `commands/` — `/linear-orchestration [task]`, an explicit slash-command handle that invokes the workflow skill (use when you'd rather trigger it directly than rely on skill-discovery); and `/reindex [repo-root]`, which rebuilds the repo-docs semantic index for the current repo (re-embeds all Markdown — run after adding/editing docs). Both require a Claude Code surface (CLI, IDE extension, or Cowork); plugin commands/skills/subagents do **not** run in the Claude Desktop *chat* app — only the bundled MCP servers do.
