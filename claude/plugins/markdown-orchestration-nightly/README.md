...
Experimental fork of `markdown-orchestration`. Same git-ignored `.orchestration/` store, same intake, explore, refine, design, plan, and converge phases. Different execution layer. Engages only when asked for by name (`/orchestrate-nightly`, or "nightly" in the request), so the stable plugin keeps owning the generic triggers.

Goal per epic, against the stable workflow on comparable work: less wall-clock and no more tokens.

Requires the stable `markdown-orchestration` plugin to stay installed. Nightly reuses its repo-docs MCP tools and its grilling, grill-with-docs, domain-modeling, and wcag-guidelines skills, and registers no MCP server of its own.
...
```bash
node claude/plugins/markdown-orchestration-nightly/bench/orchestration-cost.cjs --since 2026-09-01
```
...
- `skills/markdown-orchestration-nightly/` — dispatcher, `references/` (`routing`, `store-protocol`, `intake-design`, `execution`, `platform`, `close-out-brief`), `templates/`.
...
```bash
node --test claude/plugins/markdown-orchestration-nightly/agents/agent-contracts.test.cjs claude/plugins/markdown-orchestration-nightly/bench/orchestration-cost.test.cjs
claude plugin validate claude/plugins/markdown-orchestration-nightly
```
