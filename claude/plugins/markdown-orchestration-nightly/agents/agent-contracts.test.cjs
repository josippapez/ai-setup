...
const skill = path.join(plugin, 'skills/markdown-orchestration-nightly');
...
test('routing names only agents that exist, under the nightly namespace', () => {
  ...
  const named = [...routing.matchAll(/`markdown-orchestration-nightly:([a-z-]+)`/g)].map((m) => m[1]);
  ...
  for (const gone of removedAgents) assert.doesNotMatch(routing, new RegExp(`markdown-orchestration-nightly:${gone}\``), gone);
  // companion skills stay on the stable plugin
  for (const s of ['markdown-orchestration:grilling', 'markdown-orchestration:grill-with-docs', 'markdown-orchestration:domain-modeling']) assert.ok(routing.includes(s), s);
});
...
test('plugin is registered in the marketplace, installer, and settings, and bundles no MCP server', () => {
  ...
  assert.equal(marketplace.plugins.find((p) => p.name === 'markdown-orchestration-nightly')?.source, './claude/plugins/markdown-orchestration-nightly');
  const manifest = JSON.parse(read(plugin, '.claude-plugin/plugin.json'));
  assert.equal(manifest.name, 'markdown-orchestration-nightly');
  ...
  assert.ok(installer.includes('markdown-orchestration-nightly@ai-setup'));
  const settings = JSON.parse(read(repo, 'claude/settings.json'));
  assert.equal(settings.enabledPlugins['markdown-orchestration-nightly@ai-setup'], true);
});
