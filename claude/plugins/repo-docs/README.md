...
`claude/install.sh` installs the `codegraph` CLI (`npm i -g @colbymchenry/codegraph`). Each repo needs a one-time `codegraph init`; it writes a git-ignored `.codegraph/` directory and keeps it fresh with a file watcher. Repos without `.codegraph/` get clean "not indexed" guidance from the tool, nothing fails. `claude/settings.json` also carries CodeGraph's `codegraph prompt-hook` on `UserPromptSubmit`: for prompts that name a symbol or ask a structural question it injects the explore result up front; every other prompt is a no-op. Telemetry is off: `.mcp.json` and `claude/settings.json` set `CODEGRAPH_TELEMETRY=0`, and the installer runs `codegraph telemetry off`.
