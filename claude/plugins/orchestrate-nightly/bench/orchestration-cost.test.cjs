...
test('collectDispatches links an Agent tool_use to the agentId in its result', () => {
  const records = [
    { message: { content: [{ type: 'tool_use', name: 'Agent', id: 'tu1', input: { subagent_type: 'orchestrate-nightly:md-builder', model: 'haiku', prompt: 'x' } }] } },
  ...
  assert.deepEqual(found.abc, { type: 'orchestrate-nightly:md-builder', model: 'haiku', promptLen: 1, parent: null, project: 'proj', file: 'main.jsonl', ts: '2026-09-01T00:00:00Z' });
});
...
test('scanProjects walks main + subagent transcripts and aggregates by plugin namespace', (t) => {
  ...
  fs.writeFileSync(path.join(proj, 'sess1.jsonl'),
    line({ message: { content: [{ type: 'tool_use', name: 'Agent', id: 'a', input: { subagent_type: 'orchestrate:md-worker', model: 'sonnet' } }, { type: 'tool_use', name: 'Agent', id: 'b', input: { subagent_type: 'orchestrate-nightly:md-builder', model: 'sonnet' } }, { type: 'tool_use', name: 'Agent', id: 'c', input: { subagent_type: 'Explore' } }] } })
  ...
  assert.deepEqual(rows.map((r) => `${r.plugin}:${r.role}:${r.totalTokens}`).sort(), ['orchestrate-nightly:md-builder:550', 'orchestrate:md-worker:2200']);
  const agg = aggregate(rows);
  assert.equal(agg.length, 2);
  assert.equal(agg.find((a) => a.plugin === 'orchestrate').turns, 2);
});
