...
1. `orchestrate:grilling` — the relentless one-question-at-a-time interview that drives the session.
2. `orchestrate:domain-modeling` — the moment a term or an architectural decision is settled, write it down (glossary entry / ADR) using this skill.
