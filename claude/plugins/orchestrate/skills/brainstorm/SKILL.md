---
name: brainstorm
description: 'Pin down what to build and how, through one-question-at-a-time dialogue, before any tracking, decomposition, or code. For when the outcome or approach itself is still undecided, not just scope details.'
when_to_use: 'Any moment a request describes an outcome you have not yet nailed down — a new capability, an unclear data source or output shape, a "build me X" with no defined format, or a choice between real approaches. Triggers: "build a", "create a", "I need a way to", "can you make", "I want a tool/script/scraper/service that". Run this BEFORE Gate/Intake in the orchestrate skill whenever the outcome/approach is undefined; skip it when the outcome is already clear and only scope/AC details are missing — that gap belongs to Intake''s grilling instead.'
---
