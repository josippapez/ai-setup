...
2. Run the **full** suite (or the broadest suite that runs in reasonable time — if the full run is prohibitively slow, run the suites covering the changed areas' dependents; use `get_file_dependents`/`get_blast_radius` reasoning from the diff, and SAY in your verdict what you scoped and why — never silently narrow).
