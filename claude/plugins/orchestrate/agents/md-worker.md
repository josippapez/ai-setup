...
- A complexity signal (low | medium | high | max). It is the model the orchestrator picked for you: low=haiku, medium=sonnet, high=opus, max=fable. When you dispatch a specialist that builds or edits rather than judges, carry the same tier unless its routing row says otherwise.
...
5. **Request review (the PR):** spawn enabled gates IN PARALLEL, passing context-pack inputs verbatim plus paths and diff:
   ...
   - a **md-reviewer** (`subagent_type: orchestrate:md-reviewer`) — correctness vs acceptance criteria. **Pick its model by complexity:** **opus** when the chunk is `high` complexity OR touches security/auth, data migrations, concurrency, money, or a large/cross-cutting diff — **fable** for the highest-stakes of these; **sonnet** otherwise — **never haiku** (too weak to be a reliable verdict gate).
   - a **regression-checker** (`subagent_type: orchestrate:regression-checker`, sonnet) only when supplied `test_surface` includes a runnable existing suite; pass that suite verbatim.
...
6. **Apply status on join:** after every dispatched gate returns, all blocking gates pass → **Done**; any fail → **In Progress**. Skipped predicates are recorded decisions, not pass results.
