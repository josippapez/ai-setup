...
const claudeAgents = path.join(repo, 'claude/plugins/markdown-orchestration/agents');
const openCodeAgents = path.join(repo, 'opencode/agents');
const claudeSkill = path.join(repo, 'claude/plugins/markdown-orchestration/skills/markdown-orchestration');
const openCodeSkill = path.join(repo, 'opencode/skills/markdown-orchestration');
...
test('recursive install/package layouts preserve progressive resources', (t) => {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'markdown-orchestration-contract-'));
  t.after(() => fs.rmSync(temp, { recursive: true, force: true }));

  const stagedClaudePlugin = path.join(temp, 'claude-plugin');
  fs.cpSync(path.join(repo, 'claude/plugins/markdown-orchestration'), stagedClaudePlugin, { recursive: true });
  const marketplace = JSON.parse(fs.readFileSync(path.join(repo, '.claude-plugin/marketplace.json'), 'utf8'));
  assert.equal(marketplace.plugins.find((plugin) => plugin.name === 'markdown-orchestration')?.source, './claude/plugins/markdown-orchestration');
  ...
  const installedRoots = [
    path.join(stagedClaudePlugin, 'skills/markdown-orchestration'),
    path.join(stagedOpenCode, 'skills/markdown-orchestration'),
  ];
...
test('no stale standards-checker discovery or tooling responsibility remains', () => {
  const roots = [path.join(repo, 'claude/plugins/markdown-orchestration'), path.join(repo, 'opencode/skills/markdown-orchestration')];
  const markdown = [];
