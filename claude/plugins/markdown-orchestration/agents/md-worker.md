...
5. **Request review (the PR):** spawn IN PARALLEL via the Agent tool, passing each the explicit `{issuePath, epicDir}`, the acceptance criteria, the diff, and the validation commands. **Use the fully-qualified, plugin-namespaced `subagent_type` (the `markdown-orchestration:` prefix) — the bare name may not resolve from inside a subagent:**
   ...
   - **You CAN spawn these — spawning is platform-supported.** You hold the Agent tool, and nested subagents are allowed (fixed depth cap of 5; you are only depth 1, so a reviewer is depth 2). If a spawn is denied or errors, record it in your `relay` array and report it — **never** conclude you "lack the Agent tool," that the platform "restricts nesting to one level," or otherwise rationalize a failed spawn into a general capability limit. A failure is local (retry or relay), not a platform rule.
