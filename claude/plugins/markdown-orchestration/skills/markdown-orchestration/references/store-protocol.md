...
Normal issue flow: `Todo → In Progress → In Review → Done`, or `In Review → In Progress` on review failure. The worker is the sole issue-status writer at start, before review, and after all parallel reviewers join. The orchestrator sets status for solo docs-maintainer work and may set/reopen/cancel status during convergence, blocker resolution, or abort, but only when no append is in flight. Reviewers, checkers, scouts, planners, and docs-maintainers **never move status**; they append results only.
