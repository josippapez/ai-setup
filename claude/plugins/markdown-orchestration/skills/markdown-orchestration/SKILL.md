...
`${CLAUDE_SKILL_DIR}` is Claude Code's native absolute directory for the skill containing this `SKILL.md`. Every Read call for a bundled resource MUST use an absolute `${skillRoot}/...` path; never resolve these resources from cwd, the target repository, or a worktree. References/templates do **not** auto-load. Remembering a prior session or following a Markdown link is not sufficient.
