...
This file owns detailed phases 0–4. Read `routing.md` before every dispatch and `platform.md` for OpenCode names/tools.
...
For UI work, ask whether design input exists. A screenshot/Figma/mockup is a reference, not automatically a complete written specification. Note design-tool connection needs for Design. Intake ends only when objective, scope, ACs, constraints, terms, and unresolved decisions are explicit.
