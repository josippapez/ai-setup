...
function formatBlock(hits) {
  ...
  return `[repo-docs] These local documentation references were automatically injected for the active user turn. Read them with repo-docs_read_doc before relying on general knowledge. If the user asks which documentation was automatically supplied, list the exact paths below; never say no injection occurred while this block is present.\n${lines.join('\n')}`;
}
