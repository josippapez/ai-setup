...
- A persisted **context-pack slice** with files/reuse plus verbatim `applicable_documented_standards`, `owning_docs`, `non_test_quality_commands`, `test_surface`, and `solution_reuse_signals`, explicit empty results, and any accepted solution-reuse report. Never rediscover routing inputs.
...
3. **Docs self-check:** inspect only supplied `owning_docs`; update in-scope ones and report out-of-scope ones. Empty means skip and record why; never discover owning docs yourself.
...
5. **Request review (the PR):** spawn IN PARALLEL via the native `task` tool, passing each the explicit `{issuePath, epicDir}`, the acceptance criteria, the diff, and the validation commands. **Use the fully-qualified, OpenCode `subagent_type` (a bare agent name) — the bare name may not resolve from inside a subagent:**
   - **code-standards-checker** only when supplied `applicable_documented_standards` is non-empty; pass it verbatim. Empty means skip, never fake pass.
   - **quality-gates-checker** only when supplied `non_test_quality_commands` is non-empty; pass it verbatim.
   ...
   - **implementation-quality-reviewer** for substantive source changes only; skip docs-only, config-only, generated, and mechanical changes. Pass the preflight report/spec.
   - Each appends its own verdict. Append a routing record for every specialist: `dispatched` or `skipped: reason`.
   ...
   - **Reviewer dispatch:** use OpenCode's native `task` tool for one bounded review layer. Every blocking checker/reviewer call MUST explicitly set `background: false`, including calls issued in parallel. A response that says the task is running, queued, pending, or completing in the background is not a verdict: keep `status: In Review` until a completed `pass`/`fail` result arrives, or relay the unresolved gate if no synchronous join is available. Never record that acknowledgement as a pass or use it to set Done. If `task` is unavailable, put the review requests in `relay` for the orchestrator; never mark the chunk Done without independent review.
6. **Apply status on join:** after all dispatched gates return, all blocking gates pass → **Done**; any fail → **In Progress**. Skips are recorded decisions, not passes.
...
```json
{
  ...
  "review": { "standards": "pass | fail | skipped", "quality_gates": "pass | fail | skipped", "implementation_quality": "pass | fail | skipped", "reviewer_tier": "Sol | Luna | Terra", "verdict": "pass | fail", "regression": "pass | fail | skipped", "tests": "pass | skipped", "rounds": 1 },
